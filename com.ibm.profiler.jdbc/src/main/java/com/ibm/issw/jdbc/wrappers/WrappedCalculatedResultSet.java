...
/**
 * 
 * WrappedCalculatedResultSet
 */
public class WrappedCalculatedResultSet implements ResultSet {
	...
	private static final Logger LOG = Logger
			.getLogger(WrappedCalculatedResultSet.class.getName());
	...
	public WrappedCalculatedResultSet(ResultSet resultSet, String reference,
		...
		// accessing the result set increments the byte count.

		event.setReadSize(event.getReadSize() + ROW_READ_OVERHEAD);
    ...
    public String getString(int columnIndex) throws SQLException {
		...
		int addReadSize = safeStringLength(retVal);
		addReadSize(addReadSize);

		return retVal;
	}

	private void addReadSize(long addReadSize) {
		event.setReadSize(event.getReadSize() + addReadSize);
		event.setReadSize(event.getReadSize() + DATA_CELL_OVERHEAD);
	}
	/*
	 * (non-Javadoc)
	...
	public BigDecimal getBigDecimal(int columnIndex, int scale)
		...
		int bigDecimalLengthSafe = bigDecimalLengthSafe(bigDecimal);
		addReadSize(bigDecimalLengthSafe);

		return bigDecimal;
	}

	private int bigDecimalLengthSafe(BigDecimal bigDecimal) {
		if (bigDecimal == null) {
			return 4;
		}
		int bigDecimalLengthSafe = bigDecimal.toString().length();
		return bigDecimalLengthSafe;
	}
	/*
	 * (non-Javadoc)
    ...
    public byte[] getBytes(int columnIndex) throws SQLException {
		...
		addReadSize(getArrayLengthSafe(bytes));

		return bytes;
	}

	private int getArrayLengthSafe(byte[] bytes) {
		if (bytes == null) {
			return 4;
		}
		return bytes.length;
    ...
    public String getString(String columnName) throws SQLException {
		...
		addReadSize(safeStringLength(string));

		return string;
	}

	private int safeStringLength(String string) {
		if (string == null) {
			// null value token
			return 4;
		}
	...
	public BigDecimal getBigDecimal(String columnName, int scale)
		...
		addReadSize(bigDecimalLengthSafe(bigDecimal));
    ...
    public Object getObject(int columnIndex) throws SQLException {
		...
		addReadSize(getObjectSizeSafe(object));

		return object;
	}

	private int getObjectSizeSafe(Object object) {
		if (object == null) {
			return 4;
		}
		try {
			String string = object.toString();
			return safeStringLength(string);
		} catch (Exception ex) {
			LOG.log(Level.WARNING, "getObjectSizeSafe failed for toString", ex);
		}
		return 4;
	}
	/*
    ...
    public BigDecimal getBigDecimal(int columnIndex) throws SQLException {
		...
		addReadSize(getObjectSizeSafe(bigDecimal));
    ...
    public URL getURL(int columnIndex) throws SQLException {
		...
		addReadSize(getObjectSizeSafe(url));
	...
	public NClob getNClob(int arg0) throws SQLException {
		...
		addReadSize(getNClobSize(clob));

		return clob;
	}

	private long getNClobSize(NClob clob) {
		if (clob == null) {
			return 4;
		}
		try {
			...
			LOG.log(Level.WARNING, "failed to getNClobSize", e);
		}
		return 4;
	}
	/*
	...
	public SQLXML getSQLXML(int arg0) throws SQLException {
		...
		addReadSize(getObjectSizeSafe(sqlxml));
